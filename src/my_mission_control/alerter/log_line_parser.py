...
def parse_log_line(line) -> Optional[LogEntry]:
    ...
    """
    parts = line.strip().split(LogCfg.LOG_LINE_DELIMITER)
    if len(parts) != LogCfg.LOG_LINE_EXPECTED_FIELD_COUNT:
        logger.warning(f"Invalid line, expected {LogCfg.LOG_LINE_EXPECTED_FIELD_COUNT} fields, got {len(parts)}")
        return None

    try:
        ts_str, sat_id, rhl, yhl, yll, rll, val, cmpnt = parts
        ts = datetime.strptime(ts_str, LogCfg.LOG_LINE_TIME_FORMAT_INPUT)
